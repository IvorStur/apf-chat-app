...
function ChatComponent() {
  ...
  useEffect(() => {
    ...
    socket.on("current chat", (msg) => {
      ...
      if (msg.length > 0) {
        msg.map((theActualMsg) => {
          setMessages((msgs) => [...msgs, [theActualMsg[0], theActualMsg[1]]]); // Concatenate messages
        });
      }
    });
  ...
  const sendMessage = () => {
    if (message) {
      ...
      socket.emit("chat message", room, message, username, (feedback) => {
            ...
            <div className="flex items-start gap-2.5" key={index}>
              <div className="flex flex-col w-full max-w-[320px] leading-1.5 p-4 border-gray-200 bg-gray-100 rounded-e-xl rounded-es-xl dark:bg-gray-700">
                ...
                <p className="text-sm font-normal py-2.5 text-gray-900 dark:text-white">
                  {msg.url ? <img src={msg.url} alt="binary" /> : msg[1]}
                </p>
                <span className="text-sm font-normal text-gray-500 dark:text-gray-400">
                  Delivered
                </span>
