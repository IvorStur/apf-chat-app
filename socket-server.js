...
const chat = io.of("/chat").on("connection", (socket) => {
  ...
  socket.use((packet, next) => {
    ...
    console.log(`Received packet in chat namespace from ${socket.id}:`, packet);
    next();
  });
  ...
  socket.on("join room", (room) => {
    ...
    if (messages[room]) {
      io.to(socket.id).emit("current chat", messages[room]);
  ...
  socket.on("chat message", (room, msg, username, ackFn) => {
    console.log(username, msg);
    chat.to(room).emit("chat message", [username, msg]);
    ...
    messages[room].push([username, msg]);
  ...
  socket.on("current chat", () => {
    chat.to(room).emit("current chat", messages[room]);
  });
...
const news = io.of("/news").on("connection", (socket) => {
  ...
  socket.on("subscribe", (topic, callback) => {
    ...
    news.to(topic).emit("notification", `${socket.id} subscribed to ${topic}`);
